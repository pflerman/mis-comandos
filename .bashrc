...
export OSH='/home/pepe/.oh-my-bash'
...
# Función para reproducir música de YouTube
ytm() {
    mpv --no-video --shuffle $(yt-dlp --no-warnings --extractor-args "youtube:player_client=default" --get-url "ytsearch10:$*")
}
alias mycommands='bat ~/.mycommands.md'

# Buscar en mis comandos
findcmd() {
    local selection=$(bat ~/.mycommands.md | fzf --ansi --preview 'echo {}' --preview-window up:3:wrap)
    if [ -n "$selection" ]; then
        local cmd=$(echo "$selection" | sed 's/ *→.*//' | sed 's/^- *//')
        echo "$cmd" | wl-copy
        echo "✓ Comando copiado: $cmd"
        echo "  Pegalo con Ctrl+Shift+V"
    fi
}

# Backup de comandos a GitHub
savecmd() {
    cp ~/.mycommands.md ~/mis-configs/mycommands.md
    cd ~/mis-configs
    git add mycommands.md
    git commit -m "update $(date +%Y-%m-%d)"
    git push
    cd -
    echo "✓ Comandos guardados en GitHub"
}
